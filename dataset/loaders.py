...
def wav_data (root_path, meta_files=None):
    ...
    if meta_files is None: meta_files = glob(f"{root_path}/data_file_small.txt", recursive=True)
    meta_files.sort()
    items = []
    for meta_file in meta_files: 
        language_dir = os.path.dirname(meta_file)
        with open(meta_file, 'r', encoding='utf-8') as ttf:
            ...
            for line in ttf:
                ...
                full_audio= root_path+"/Multilingual_Text_to_Speech/data/wav_data/"+speaker_name
                #print(full_audio)
